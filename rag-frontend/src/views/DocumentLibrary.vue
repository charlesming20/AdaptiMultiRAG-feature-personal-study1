...
const addDocument = async () => {
  try {
    ...
    // 文件上传：直接交给后端解析并写入 Milvus / LightRAG
    if (documentForm.type === "file") {
      ...
      const uploadResponse = await knowledgeAPI.uploadDocumentFile({
      ...
      if (uploadResponse.status === 200) {
        ElMessage.success(uploadResponse.msg || "文件已上传，后台正在解析并生成知识图谱");
        resetDocumentForm();
        showAddDocumentDialog.value = false;
        await selectLibrary(selectedLibrary.value);
        return;
      ...
      throw new Error(uploadResponse.msg || "文件上传入库失败");
    }
