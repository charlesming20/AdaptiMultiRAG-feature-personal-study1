      ...
      await sendMessageStream(
        ...
        (data) => {
          ...
          if (data.type === 'start') {
            ...
            if (lastMessage && lastMessage.role === 'node_update' && lastMessage.node_name === data.node_name) {
              ...
              if (currentConversation.value && !currentConversation.value.saved && currentConversation.value.messages?.length) {
                const localLastMessage = currentConversation.value.messages[currentConversation.value.messages.length - 1]
                if (localLastMessage && localLastMessage.role === 'node_update' && localLastMessage.node_name === data.node_name) {
                  currentConversation.value.messages.splice(currentConversation.value.messages.length - 1, 1, updatedMessage)
                }
              }
              return
            }
