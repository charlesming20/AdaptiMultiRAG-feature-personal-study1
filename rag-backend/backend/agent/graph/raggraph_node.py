...
class RAGNodes:
    ...
    def _invoke_json(self, prompt: str, schema_model, example: str):
        json_prompt = f"""{prompt}

请严格只返回一个 JSON 对象，不要输出 Markdown，不要使用 ```json 代码块，不要添加解释文字。
JSON 字段必须符合下面示例：
{example}
"""
    ...
    def _parse_json_response(self, raw_content: str) -> dict:
        ...
        code_block = re.search(r"```(?:json)?\s*(.*?)\s*```", content, re.DOTALL | re.IGNORECASE)
        if code_block:
        ...
        raise ValueError(f"无法解析模型 JSON 输出: {preview}")
        ...
        try:
            ...
            try:
                decision = self._invoke_json(
                    prompt,
                    RetrievalNeedDecision,
                    '{"need_retrieval": true, "extracted_question": "用户核心问题", "reasoning": "判断理由"}'
                )
    ...
    def expand_subquestions_node(self, state: RAGGraphState, runtime: Runtime[RAGContext]) -> RAGGraphState:
        ...
        try:
            ...
            # 使用结构化输出调用LLM
            expansion_result = self._invoke_json(
                prompt,
                SubquestionExpansion,
    ...
    def classify_question_type_node(self, state: RAGGraphState, runtime: Runtime[RAGContext]) -> RAGGraphState:
        ...
        if state["retrieval_mode"] == RetrievalMode.AUTO:
            ...
            try:
                ...
                graph_keywords = [
                    "relationship", "relationships", "relation", "related", "connected", "connection",
                    "between", "associate", "associated", "cause", "causal", "why", "path",
                    "entity", "entities", "knowledge graph", "graph retrieval", "graph",
                    "关系", "关联", "联系", "连接", "因果", "原因", "为什么",
                    "路径", "实体", "图谱", "知识图谱", "影响", "推动"
                ]
                question_lower = original_question.lower()
                if any(keyword in question_lower for keyword in graph_keywords):
                    state["retrieval_mode"] = RetrievalMode.GRAPH_ONLY
                    state["retrieval_mode_reason"] = "问题包含关系、因果或实体连接类关键词，优先使用图数据库检索"
                    self.logger.info(f"规则判断：使用图检索 - {state['retrieval_mode_reason']}")
                    return state
                ...
                try:
                    decision = self._invoke_json(
                        prompt,
                        RetrievalTypeDecision,
                        '{"retrieval_type": "vector_only", "reasoning": "选择该检索类型的理由"}'
                    )
                    ...
                    # 解析失败时默认使用向量检索
                    state["retrieval_mode"] = RetrievalMode.VECTOR_ONLY
                    state["retrieval_mode_reason"] = f"LLM结构化输出调用失败: {str(parse_error)}，默认使用向量检索"

            except Exception as e:
                self.logger.error(f"检索类型判断失败: {e}")
                # 出错时默认使用向量检索
                state["retrieval_mode"] = RetrievalMode.VECTOR_ONLY
                state["retrieval_mode_reason"] = f"检索类型判断失败: {str(e)}，默认使用向量检索"
        else:
