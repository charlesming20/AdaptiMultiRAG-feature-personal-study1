...
def init_models():
    ...
    register_model_provider(
        provider_name="zhipu",
        chat_model="openai",
        base_url=os.getenv("LLM_DASHSCOPE_API_BASE", "https://open.bigmodel.cn/api/paas/v4")
    )

    chat_model = load_chat_model(
        f"zhipu:{os.getenv('LLM_DASHSCOPE_CHAT_MODEL', 'glm-5.2')}",
        api_key=(
    ...
    register_embeddings_provider(
        ...
        base_url=os.getenv("VECTOR_DASHSCOPE_API_BASE", "https://open.bigmodel.cn/api/paas/v4")
    )

    print("加载向量模型...")
    embeddings_model = load_embeddings(
        f"ali:{os.getenv('VECTOR_DASHSCOPE_EMBEDDING_MODEL', 'embedding-3')}",
        api_key=(
