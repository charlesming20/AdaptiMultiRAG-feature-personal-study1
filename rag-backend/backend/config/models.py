...
def initialize_chat_model():
    ...
    # 从环境变量获取大语言模型配置
    api_key = (
        os.getenv("LLM_DASHSCOPE_API_KEY")
        or os.getenv("BIGMODEL_API_KEY")
        or os.getenv("VECTOR_DASHSCOPE_API_KEY")
        or os.getenv("DASHSCOPE_API_KEY")
    )
    api_base = os.getenv("LLM_DASHSCOPE_API_BASE", "https://open.bigmodel.cn/api/paas/v4")
    model_name = os.getenv("LLM_DASHSCOPE_CHAT_MODEL", "glm-5.2")
...
def initialize_embeddings_model():
    ...
    # 从环境变量获取向量模型配置
    api_base = os.getenv("VECTOR_DASHSCOPE_API_BASE", "https://open.bigmodel.cn/api/paas/v4")
    embedding_model = os.getenv("VECTOR_DASHSCOPE_EMBEDDING_MODEL", "embedding-3")
    ...
    # 从环境变量获取向量模型 API Key
    api_key = (
        os.getenv("VECTOR_DASHSCOPE_API_KEY")
        or os.getenv("BIGMODEL_API_KEY")
        or os.getenv("DASHSCOPE_API_KEY")
    )
    if not api_key:
